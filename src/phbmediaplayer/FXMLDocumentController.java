...
public class FXMLDocumentController implements Initializable {
    ...
    private void playMedia(int index){
        ...
        seek.setOnMouseClicked((MouseEventl) -> {
            mediaPlayer.seek(Duration.seconds(seek.getValue()));
        });
        seek.setOnMouseDragExited((MouseEventl) -> {
            mediaPlayer.seek(Duration.seconds(seek.getValue()));
        });
        mediaPlayer.play();
        PLAY = 1;
        Image imagePause = new Image(getClass().getResourceAsStream("/image/00.jpg"));
    ...
    private void pausePlay(ActionEvent event){
        if(!playlist.isEmpty()){
            ...
            dialog.setOnCloseRequest((DialogEvent eventl) ->{
                dialog.close();
            });
